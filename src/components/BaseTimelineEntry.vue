...
<template>
  ...
  >
    <div class="flex gap-4">
      <div class="z-10 text-xl group-hover:grayscale-0 sm:-ml-11" :class="{ grayscale: !experience.isActive }">
        {{ experience.icon }}
      </div>
      <div>
        ...
        <ul v-if="experience.subExperiences" class="ml-4 mt-2 list-disc">
          <li v-for="subExperience in experience.subExperiences" class="flex items-center gap-1">
            <span>{{ subExperience.name }} -</span>
            <time class="text-xs tracking-wide text-gray-400 dark:text-gray-400">{{ subExperience.date }}</time>
